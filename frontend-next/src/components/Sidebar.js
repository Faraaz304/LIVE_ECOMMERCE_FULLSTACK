...
const Sidebar = ({ sidebarOpen, handleGoLive, handleLogout, userRole = 'user', userName = 'User' }) => {
  const pathname = usePathname();
  
  // Get navigation items based on user role
  const navItems = navigationConfig[userRole] || navigationConfig.user;
  
  // Show "Go Live" button only for sellers
  const showGoLiveButton = userRole === 'seller';
