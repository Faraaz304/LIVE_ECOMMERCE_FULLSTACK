...
const EditProductPage = () => {
  ...
  const { productid: routeProductId } = params;               // ← CHANGE: correct param name
  ...
  // ← NEW: separate loading flag for the delete operation (optional)
  const [deleteLoading, setDeleteLoading] = useState(false);   // ← CHANGE
  ...
  const handleDeleteProduct = async () => {
    ...
    setSubmitError(null);
    setDeleteLoading(true);                         // ← CHANGE: start loading
    ...
    try {
      ...
      if (result.success) {
        ...
        setSubmitError(result.error || 'Failed to delete product');
      }
    } catch (err) {
      console.error('Error deleting product:', err);
      setSubmitError(err.message || 'Failed to delete product');
    } finally {
      setDeleteLoading(false);                      // ← CHANGE: stop loading
    }
  };
        ...
        <form onSubmit={handleUpdateProduct}>
              ...
              <div className="bg-white rounded-xl border border-[#e5e7eb] p-6">
                ...
                <div className="mb-5">
                  ...
                  <textarea
                    ...
                    onChange={handleInputChange}
                    required
                    // ← CHANGE: removed undefined `isSubmitting`
                    disabled={isLoading}
                  ></textarea>
          ...
          <div className="bg-white border-t border-[#e5e7eb] p-5 flex justify-between items-center mt-6 rounded-xl">
            ...
            <Button
              ...
              size="lg"
              className="bg-gradient-to-r from-[#667eea] to-[#764ba2] hover:opacity-90"
              disabled={isLoading}               // ← uses hook's isLoading
            >
